...
export const ModelSelector = ({
  ...
  const isModelsReady = runtimeState?.modelsState === 'ready';
  ...
  const modelStatusMessage =
    modelLoading === 'all' || modelLoading === provider?.name
      ? 'Загружаем модели провайдера...'
      : runtimeState?.modelsState === 'ready'
        ? undefined
        : runtimeState?.warningMessage || 'Модели недоступны. Подключите API.';
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-stretch gap-2">
        {/* Provider Combobox */}
        <div
          className="relative flex h-[52px] w-[52px] shrink-0"
          onKeyDown={handleProviderKeyDown}
          ref={providerDropdownRef}
        >
          <div
            className={classNames(
              'h-full w-full rounded-lg border border-bolt-elements-borderColor',
            ...
            aria-label={provider?.name || 'Select provider'}
            title={provider?.name || 'Select provider'}
            tabIndex={0}
          >
            <div className="flex h-full w-full items-center justify-center gap-0.5">
              <ProviderIcon providerName={provider?.name} className="w-4 h-4 shrink-0 text-bolt-elements-textPrimary" />
              <span className="sr-only">{provider?.name || 'Select provider'}</span>
              <div
                className={classNames(
          ...
          {isProviderDropdownOpen && (
            <div
              className="absolute left-0 top-[calc(100%+6px)] z-20 w-72 max-w-[calc(100vw-2rem)] py-1 rounded-lg border border-bolt-elements-borderColor bg-bolt-elements-background-depth-2 shadow-lg"
              role="listbox"
              id="provider-listbox"
            >
        ...
        {/* Model Combobox */}
        <div className="relative flex flex-1 min-w-0" onKeyDown={handleModelKeyDown} ref={modelDropdownRef}>
          <div
            className={classNames(
              'h-[52px] w-full rounded-lg border border-bolt-elements-borderColor px-4',
              'bg-bolt-elements-prompt-background text-bolt-elements-textPrimary',
              'focus-within:outline-none focus-within:ring-2 focus-within:ring-bolt-elements-focus',
              'transition-all cursor-pointer',
            ...
            >
              ...
              >
                {modelLoading === 'all' || modelLoading === provider?.name ? (
                  ...
                  <div className="px-3 py-2 text-sm text-bolt-elements-textTertiary">{modelStatusMessage}</div>
                ) : (
        ...
        {settingsTrigger && <div className="flex shrink-0 items-center">{settingsTrigger}</div>}
      </div>
    </div>
  );
};
